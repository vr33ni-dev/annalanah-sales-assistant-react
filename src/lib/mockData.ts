// Mock data for Lovable preview environment
import type {
  Client,
  Contract,
  ContractUpsell,
  Lead,
  SalesProcess,
  Stage,
  Comment,
  CommentEntityType,
} from "./api";

export const isLovablePreview = () => {
  if (typeof window === "undefined") return false;
  const host = window.location.hostname;
  return (
    host.includes("lovableproject.com") ||
    host.includes("lovable.app") ||
    host.includes("webcontainer")
  );
};

export const mockClients: Client[] = [
  {
    id: 1,
    name: "Anna Schmidt",
    email: "anna@example.com",
    phone: "123456",
    source: "organic",
    source_stage_name: null,
    status: "active",
    completed_at: "2026-04-23",
  },
  {
    id: 2,
    name: "Max Müller",
    email: "max@example.com",
    phone: "987654",
    source: "paid",
    source_stage_name: "Facebook Ads September",
    status: "active",
    completed_at: "2026-03-15",
  },
  {
    id: 3,
    name: "Moritz Mustermann",
    email: "mo@example.com",
    phone: "912345",
    source: "paid",
    source_stage_name: "Facebook Ads September",
    status: "follow_up_scheduled",
  },
  {
    id: 4,
    name: "Maria Mustermann",
    email: "ma@example.com",
    phone: "912345",
    source: "paid",
    source_stage_name: "Facebook Ads September",
    status: "lost",
  },
];

export const mockContracts: Contract[] = [
  {
    id: 1,
    client_id: 1,
    client_name: "Anna Schmidt",
    sales_process_id: 1,
    created_at: "2025-09-01T09:00:00Z",
    start_date: "2025-10-01",
    end_date: "2026-03-31",
    duration_months: 6,
    revenue_total: 4800,
    payment_frequency: "monthly",
    base_monthly_amount: 800,
    next_due_date: null,
  },
  {
    id: 2,
    client_id: 2,
    client_name: "Max Müller",
    sales_process_id: 2,
    created_at: "2025-09-10T10:30:00Z",
    start_date: "2025-09-23",
    end_date: "2026-03-22",
    duration_months: 6,
    revenue_total: 6000,
    payment_frequency: "bi-monthly",
    base_monthly_amount: 1000,
    next_due_date: null,
  },
  {
    id: 3,
    client_id: 3,
    client_name: "Moritz Mustermann",
    sales_process_id: 3,
    created_at: "2025-03-01T08:00:00Z",
    start_date: "2025-03-23",
    end_date: "2026-03-22",
    duration_months: 12,
    revenue_total: 12000,
    payment_frequency: "bi-yearly",
    base_monthly_amount: 1000,
    next_due_date: null,
    chain: [
      {
        id: 3,
        client_id: 3,
        client_name: "Moritz Mustermann",
        sales_process_id: 3,
        created_at: "2025-03-01T08:00:00Z",
        start_date: "2025-03-23",
        end_date: "2026-03-22",
        duration_months: 12,
        revenue_total: 12000,
        payment_frequency: "bi-yearly",
        base_monthly_amount: 1000,
        next_due_date: null,
      },
      {
        id: 4,
        client_id: 3,
        client_name: "Moritz Mustermann",
        sales_process_id: 3,
        created_at: "2026-02-15T08:00:00Z",
        start_date: "2026-03-23",
        end_date: "2027-03-22",
        duration_months: 12,
        revenue_total: 12000,
        payment_frequency: "bi-yearly",
        base_monthly_amount: 1000,
        next_due_date: null,
      },
    ],
  },
  {
    id: 4,
    client_id: 3,
    client_name: "Moritz Mustermann",
    sales_process_id: 3,
    created_at: "2026-02-15T08:00:00Z",
    start_date: "2026-03-23",
    end_date: "2027-03-22",
    duration_months: 12,
    revenue_total: 12000,
    payment_frequency: "bi-yearly",
    base_monthly_amount: 1000,
    next_due_date: null,
    chain: [
      {
        id: 3,
        client_id: 3,
        client_name: "Moritz Mustermann",
        sales_process_id: 3,
        created_at: "2025-03-01T08:00:00Z",
        start_date: "2025-03-23",
        end_date: "2026-03-22",
        duration_months: 12,
        revenue_total: 12000,
        payment_frequency: "bi-yearly",
        base_monthly_amount: 1000,
        next_due_date: null,
      },
      {
        id: 4,
        client_id: 3,
        client_name: "Moritz Mustermann",
        sales_process_id: 3,
        created_at: "2026-02-15T08:00:00Z",
        start_date: "2026-03-23",
        end_date: "2027-03-22",
        duration_months: 12,
        revenue_total: 12000,
        payment_frequency: "bi-yearly",
        base_monthly_amount: 1000,
        next_due_date: null,
      },
    ],
  },
];

export const mockSalesProcesses: SalesProcess[] = [
  {
    id: 1,
    client_id: 1,
    client_name: "Anna Schmidt",
    client_email: "anna@example.com",
    client_phone: "123456",
    client_source: "organic",
    stage: "closed",
    follow_up_date: "2026-03-04",
    follow_up_result: true,
    closed: true,
    revenue: 4800,
    stage_id: null,
    created_at: "2026-02-01T09:00:00Z",
  },
  {
    id: 2,
    client_id: 2,
    client_name: "Max Müller",
    client_email: "max@example.com",
    client_phone: "987654",
    client_source: "paid",
    stage: "closed",
    follow_up_date: "2026-02-28",
    follow_up_result: true,
    closed: true,
    revenue: 6000,
    stage_id: 1,
    created_at: "2026-02-05T10:30:00Z",
  },
  {
    id: 3,
    client_id: 3,
    client_name: "Moritz Mustermann",
    client_email: "mo@example.com",
    client_phone: "912345",
    client_source: "paid",
    stage: "follow_up",
    follow_up_date: "2026-03-15",
    follow_up_result: true,
    closed: false,
    revenue: 5400,
    stage_id: 1,
    created_at: "2026-02-06T08:00:00Z",
  },
  {
    id: 4,
    client_id: 4,
    client_name: "Maria Mustermann",
    client_email: "ma@example.com",
    client_phone: "912345",
    client_source: "paid",
    stage: "lost",
    follow_up_date: "2026-03-10",
    follow_up_result: false,
    closed: false,
    revenue: null,
    stage_id: 1,
    created_at: "2026-02-07T09:00:00Z",
  },
  {
    id: 3,
    client_id: 3,
    client_name: "Peter Weber",
    client_email: "peter@example.com",
    client_phone: "+49 345 678901",
    client_source: "paid",
    stage: "closed",
    follow_up_date: "2026-01-20",
    follow_up_result: true,
    closed: true,
    revenue: 9600,
    stage_id: 2,
    created_at: "2026-01-04T11:00:00Z",
  },
  {
    id: 4,
    client_id: 4,
    client_name: "Julia Fischer",
    client_email: "julia@example.com",
    client_phone: "+49 456 789012",
    client_source: "organic",
    stage: "follow_up",
    follow_up_date: "2026-02-18",
    follow_up_result: null,
    closed: false,
    revenue: null,
    stage_id: null,
    created_at: "2026-01-06T08:00:00Z",
  },
  {
    id: 5,
    client_id: 5,
    client_name: "Thomas Müller",
    client_email: "thomas@example.com",
    client_phone: "+49 567 890123",
    client_source: "paid",
    stage: "closed",
    follow_up_date: "2026-01-25",
    follow_up_result: true,
    closed: true,
    revenue: 6000,
    stage_id: 1,
    created_at: "2026-01-07T09:30:00Z",
  },
  {
    id: 6,
    client_id: 6,
    client_name: "Sarah Braun",
    client_email: "sarah@example.com",
    client_phone: "+49 678 901234",
    client_source: "paid",
    stage: "lost",
    follow_up_date: "2026-01-30",
    follow_up_result: true,
    closed: false,
    revenue: null,
    stage_id: 3,
    created_at: "2026-01-08T10:15:00Z",
  },
  {
    id: 7,
    client_id: 6,
    client_name: "Lisa Wagner",
    client_email: "lisa@example.com",
    client_phone: "+49 789 012345",
    client_source: "organic",
    stage: "closed",
    follow_up_date: "2026-02-01",
    follow_up_result: true,
    closed: true,
    revenue: 5400,
    stage_id: null,
    created_at: "2026-01-09T12:00:00Z",
  },
  {
    id: 8,
    client_id: 7,
    client_name: "Klaus Bauer",
    client_email: "klaus@example.com",
    client_phone: "+49 890 123456",
    client_source: "paid",
    stage: "closed",
    follow_up_date: "2026-02-03",
    follow_up_result: true,
    closed: true,
    revenue: 7200,
    stage_id: 2,
    created_at: "2026-01-10T14:00:00Z",
  },
  {
    id: 9,
    client_id: 8,
    client_name: "Maria Hoffmann",
    client_email: "maria@example.com",
    client_phone: "+49 901 234567",
    client_source: "organic",
    stage: "closed",
    follow_up_date: "2026-02-05",
    follow_up_result: true,
    closed: true,
    revenue: 4800,
    stage_id: null,
    created_at: "2026-01-12T09:00:00Z",
  },
  {
    id: 10,
    client_id: 9,
    client_name: "Stefan Klein",
    client_email: "stefan@example.com",
    client_phone: "+49 012 345678",
    client_source: "paid",
    stage: "closed",
    follow_up_date: "2026-02-08",
    follow_up_result: true,
    closed: true,
    revenue: 8400,
    stage_id: 3,
    created_at: "2026-01-14T10:45:00Z",
  },
  {
    id: 11,
    client_id: 10,
    client_name: "Sabine Wolf",
    client_email: "sabine@example.com",
    client_phone: "+49 111 222333",
    client_source: "organic",
    stage: "closed",
    follow_up_date: "2026-02-10",
    follow_up_result: true,
    closed: true,
    revenue: 6600,
    stage_id: null,
    created_at: "2026-01-16T11:20:00Z",
  },
  {
    id: 12,
    client_id: 11,
    client_name: "Michael Schäfer",
    client_email: "michael@example.com",
    client_phone: "+49 222 333444",
    client_source: "paid",
    stage: "closed",
    follow_up_date: "2026-02-12",
    follow_up_result: true,
    closed: true,
    revenue: 5100,
    stage_id: 1,
    created_at: "2026-01-18T08:30:00Z",
  },
  {
    id: 13,
    client_id: 12,
    client_name: "Nicole Braun",
    client_email: "nicole@example.com",
    client_phone: "+49 333 444555",
    client_source: "organic",
    stage: "closed",
    follow_up_date: "2026-02-14",
    follow_up_result: true,
    closed: true,
    revenue: 7800,
    stage_id: null,
    created_at: "2026-01-20T09:10:00Z",
  },
  {
    id: 14,
    client_id: 13,
    client_name: "Andreas Richter",
    client_email: "andreas@example.com",
    client_phone: "+49 444 555666",
    client_source: "paid",
    stage: "closed",
    follow_up_date: "2026-02-16",
    follow_up_result: true,
    closed: true,
    revenue: 9000,
    stage_id: 2,
    created_at: "2026-01-22T10:00:00Z",
  },
  {
    id: 15,
    client_id: 14,
    client_name: "Petra Lange",
    client_email: "petra@example.com",
    client_phone: "+49 555 666777",
    client_source: "organic",
    stage: "lost",
    follow_up_date: "2026-01-28",
    follow_up_result: true,
    closed: false,
    revenue: null,
    stage_id: null,
    created_at: "2026-01-24T13:00:00Z",
  },
  {
    id: 16,
    client_id: 15,
    client_name: "Florian Meier",
    client_email: "florian@example.com",
    client_phone: "+49 666 777888",
    client_source: "organic",
    stage: "initial_contact",
    follow_up_date: "2026-02-18",
    follow_up_result: null,
    closed: false,
    revenue: null,
    stage_id: null,
    created_at: "2026-01-26T09:00:00Z",
  },
  {
    id: 17,
    client_id: 16,
    client_name: "Claudia Neumann",
    client_email: "claudia@example.com",
    client_phone: "+49 777 888999",
    client_source: "paid",
    stage: "initial_contact",
    follow_up_date: "2026-02-20",
    follow_up_result: null,
    closed: false,
    revenue: null,
    stage_id: 1,
    created_at: "2026-02-10T08:00:00Z",
  },
];

export const mockStages: Stage[] = [
  {
    id: 1,
    name: "Facebook Ads September",
    date: "2026-03-24",
    ad_budget: 2000,
    registrations: 50,
    participants: 30,
  },
  {
    id: 2,
    name: "Workshop Q2 2024",
    date: "2024-06-20",
    ad_budget: 3000,
    registrations: 52,
    participants: 38,
  },
  {
    id: 3,
    name: "Workshop Q3 2024",
    date: "2024-09-18",
    ad_budget: 2800,
    registrations: 48,
    participants: 35,
  },
  {
    id: 4,
    name: "Workshop Q4 2024",
    date: "2024-12-12",
    ad_budget: 3200,
    registrations: 55,
    participants: 40,
  },
  {
    id: 5,
    name: "Workshop Q1 2025",
    date: "2025-03-20",
    ad_budget: 3500,
    registrations: 30,
    participants: null,
  },
];

export const mockLeads: Lead[] = [
  {
    id: 1,
    name: "Peter Beispiel",
    email: "peter@lead.de",
    phone: "555987654",
    source: "organic",
    source_stage_id: null,
    source_stage_name: null,
    converted: false,
    converted_at: null,
    converted_client_id: null,
    created_at: "2026-01-01T10:00:00Z",
  },
  {
    id: 2,
    name: "Laura Beispiel",
    email: "laura@example.com",
    phone: "444333333",
    source: "paid",
    source_stage_id: 1,
    source_stage_name: "Facebook Ads September",
    converted: false,
    converted_at: null,
    converted_client_id: null,
    created_at: "2026-01-03T14:30:00Z",
  },
  {
    id: 3,
    name: "Test Lead",
    email: "test@example.com",
    phone: null,
    source: "organic",
    source_stage_id: null,
    source_stage_name: null,
    converted: false,
    converted_at: null,
    converted_client_id: null,
    created_at: "2026-01-05T09:15:00Z",
  },
  {
    id: 4,
    name: "Converted Lead",
    email: "conv@example.com",
    phone: "111222333",
    source: "organic",
    source_stage_id: null,
    source_stage_name: null,
    converted: true,
    converted_at: "2026-01-07T11:00:00Z",
    converted_client_id: 1,
    created_at: "2026-01-06T08:00:00Z",
  },
  {
    id: 5,
    name: "Sandra Neumann",
    email: "sandra@neumann.de",
    phone: "0176123456",
    source: "paid",
    source_stage_id: 2,
    source_stage_name: "Workshop Q2 2024",
    converted: false,
    converted_at: null,
    converted_client_id: null,
    created_at: "2026-01-09T16:45:00Z",
  },
];

export const mockUpsells: ContractUpsell[] = [
  {
    id: 1,
    sales_process_id: 3,
    client_id: 3,
    upsell_date: "2026-02-18",
    upsell_result: "verlaengerung",
    upsell_revenue: 12000,
    previous_contract_id: 3,
    new_contract_id: 4,
    created_at: "2026-02-18T10:00:00Z",
    updated_at: "2026-02-18T10:00:00Z",
    contract_start_date: "2026-03-23",
    contract_duration_months: 12,
    contract_frequency: "bi-yearly",
  },
];

/**
 * In-memory upsell store for Lovable preview mode.
 * Allows users to add/update upsell entries without a backend.
 */
const mockUpsellStore: ContractUpsell[] = [...mockUpsells];

export const getMockUpsellsForSalesProcess = (
  salesProcessId: number,
): ContractUpsell[] =>
  mockUpsellStore.filter((u) => u.sales_process_id === salesProcessId);

export const upsertMockUpsell = (
  salesProcessId: number,
  payload: Partial<ContractUpsell>,
): ContractUpsell => {
  const now = new Date().toISOString();
  // Match the backend behaviour: only an upsell that is still pending
  // (no result decided yet) AND has not produced a new contract is updated
  // in place. A decided upsell (verlaengerung / keine_verlaengerung) is
  // considered finalised, so a follow-up "Upsell planen" creates a new row.
  const existingIndex = mockUpsellStore.findIndex(
    (u) =>
      u.sales_process_id === salesProcessId &&
      u.new_contract_id == null &&
      !u.upsell_result,
  );
  if (existingIndex >= 0) {
    const existing = mockUpsellStore[existingIndex];
    const updated: ContractUpsell = {
      ...existing,
      ...payload,
      sales_process_id: salesProcessId,
      updated_at: now,
    };
    mockUpsellStore[existingIndex] = updated;
    return updated;
  }
  const created: ContractUpsell = {
    id: Date.now(),
    sales_process_id: salesProcessId,
    client_id: payload.client_id ?? 0,
    upsell_date: payload.upsell_date ?? null,
    upsell_result: payload.upsell_result ?? null,
    upsell_revenue: payload.upsell_revenue ?? null,
    previous_contract_id: payload.previous_contract_id ?? null,
    new_contract_id: payload.new_contract_id ?? null,
    created_at: now,
    updated_at: now,
    contract_start_date: payload.contract_start_date ?? null,
    contract_duration_months: payload.contract_duration_months ?? null,
    contract_frequency: payload.contract_frequency ?? null,
  };
  mockUpsellStore.push(created);
  return created;
};

export const mockUpsellAnalytics = {
  verlaengerung_count: 1,
  keine_verlaengerung_count: 1,
  scheduled_count: 0,
  verlaengerungsquote: 50,
  umsatz_sum: 6000,
  revenue_by_month: [],
};

export const mockDashboardKPIs = {
  total_revenue: 10800,
  new_customer_revenue: 4800,
  renewal_revenue: 6000,
  won_new_count: 1,
  decided_new_count: 2,
  closing_rate_new: 50,
  verlaengerungsquote: 50,
  active_contracts_count: 5,
  clv_active_clients: 54000,
  clv_all_time: 54000,
  active_revenue: 10800,
  avg_vertragswert: 2160,
  avg_clv_per_contract: 10800,
};

// Mock cashflow forecast data - aligned with contracts above
// Max: 1000/mo (ended), Anna: 750/mo (ended), Peter: 800/mo (until Sep 2025), Thomas: 1000/mo (until Jun 2025)
export const mockCashflowForecast = [
  { month: "2026-02", confirmed: 1800, potential: 0 }, // Anna (800) + Max (1000) - example
  { month: "2026-03", confirmed: 1400, potential: 0 }, // Anna (800) + Moritz (0 confirmed this month example)
  { month: "2026-04", confirmed: 800, potential: 0 },
  { month: "2026-05", confirmed: 800, potential: 0 },
  { month: "2026-06", confirmed: 800, potential: 0 },
  { month: "2026-07", confirmed: 800, potential: 0 },
  { month: "2026-08", confirmed: 800, potential: 0 },
  { month: "2026-09", confirmed: 1000, potential: 0 },
  { month: "2026-10", confirmed: 0, potential: 0 },
  { month: "2026-11", confirmed: 0, potential: 0 },
  { month: "2026-12", confirmed: 0, potential: 0 },
];

// Mock application settings (app_settings table)
export const mockAppSettings = {
  potential_months: 6,
  avg_revenue_per_contract: 600,
};

// Mock comments data
export const mockComments: Comment[] = [
  {
    id: 1,
    entity_type: "client",
    entity_id: 1,
    client_id: 1,
    body: "Sehr engagierter Kunde, hat großes Interesse an unserem Coaching-Programm gezeigt.",
    created_at: "2026-01-11T09:30:00Z",
  },
  {
    id: 2,
    entity_type: "client",
    entity_id: 1,
    client_id: 1,
    body: "Telefongespräch geführt - möchte in Q2 erweitern.",
    created_at: "2026-01-12T14:20:00Z",
  },
  {
    id: 3,
    entity_type: "client",
    entity_id: 2,
    client_id: 2,
    body: "Erste Beratung war sehr positiv. Follow-up nächste Woche geplant.",
    created_at: "2026-01-13T11:00:00Z",
  },
  {
    id: 4,
    entity_type: "contract",
    entity_id: 1,
    client_id: 1,
    body: "Vertrag wurde pünktlich unterzeichnet. Alle Unterlagen vollständig.",
    created_at: "2026-01-02T10:00:00Z",
  },
  {
    id: 5,
    entity_type: "contract",
    entity_id: 1,
    client_id: 1,
    body: "Kunde ist zufrieden mit dem Fortschritt. Verlängerung wahrscheinlich.",
    created_at: "2026-01-16T16:30:00Z",
  },
  {
    id: 6,
    entity_type: "contract",
    entity_id: 3,
    client_id: 2,
    body: "Zahlungserinnerung wurde versendet.",
    created_at: "2026-01-05T09:00:00Z",
  },
  {
    id: 7,
    entity_type: "sales_process",
    entity_id: 1,
    client_id: 1,
    body: "Erstkontakt per E-Mail. Interesse an Premium-Paket.",
    created_at: "2026-01-02T08:45:00Z",
  },
  {
    id: 8,
    entity_type: "sales_process",
    entity_id: 1,
    client_id: 1,
    body: "Abschluss erfolgreich! Kunde hat 12-Monats-Vertrag gewählt.",
    created_at: "2026-01-02T11:30:00Z",
  },
  {
    id: 9,
    entity_type: "sales_process",
    entity_id: 4,
    client_id: 2,
    body: "Follow-up Call angesetzt für nächste Woche.",
    created_at: "2026-01-18T15:00:00Z",
  },
  {
    id: 10,
    entity_type: "sales_process",
    entity_id: 3,
    client_id: 2,
    body: "Angebot wurde versendet. Wartet auf Rückmeldung.",
    created_at: "2026-01-14T10:15:00Z",
  },
];

// Helper to get mock comments for a specific entity
export const getMockCommentsForEntity = (
  entityType: CommentEntityType,
  entityId: number,
): Comment[] => {
  return mockComments.filter(
    (c) => c.entity_type === entityType && c.entity_id === entityId,
  );
};

// Helper to get all mock comments for a client (across entity types)
export const getMockCommentsForClient = (clientId: number): Comment[] => {
  return mockComments
    .filter((c) => c.client_id === clientId)
    .sort((a, b) => (b.created_at ?? "").localeCompare(a.created_at ?? ""));
};
